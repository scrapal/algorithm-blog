...
![KMP 失配后利用最长相同真前后缀移动模式串](./images/kmp/mismatch-shift.svg)
...
失配前已经匹配了 `ABABA`，所以此时 $j=5$。它的最长相同真前后缀是 `ABA`，长度为 `nxt[5]=3`。把开头的 `ABA` 移到末尾的 `ABA` 上，这 $3$ 个字符肯定还是匹配的，于是直接令 `j=nxt[5]=3`。
...
如果 `p[j+1]==p[i]`，原来的前后缀就可以延长一个字符，设置 `nxt[i]=j+1`。比如计算 `p="ababa"` 的 `nxt[5]`，已经知道 `nxt[4]=2`，长度为 $2$ 的相同真前后缀是 `ab`。比较：
...
两个字符相同，`ab` 可以延长成 `aba`，所以 `nxt[5]=3`。
...
![计算 nxt[6] 时不断缩短前后缀的过程](./images/kmp/fallback.svg)

最后没有找到能够继续延长的前后缀，所以 `nxt[6]=0`。代码中的 `while` 做的就是这个不断缩短前后缀的过程。
...
![利用最长相同真前后缀求最短循环节](./images/kmp/shortest-period.svg)
