...
比如原串是 `AABABAB`，模式串是 `ABA`。下面的动画演示了每次加入字符时，栈和 $j$ 的变化：

<KmpCensorAnimation />

读到第四个字符时，栈顶第一次拼出 `ABA`。删掉它以后，栈里还剩下最开始的 `A`，所以 $j$ 不能直接清零，而是要恢复成这个 `A` 保存的匹配长度 $1$。接着读入 `B`、`A`，栈顶又拼出一个 `ABA`，删掉后栈空了，$j$ 也恢复为 $0$。最后一个 `B` 和模式串开头的 `A` 不同，直接留在栈里，所以答案就是 `B`。就算一次删除让原本分开的两部分接到了一起，也可以从新栈顶保存的状态继续匹配，不需要再从头扫描。
