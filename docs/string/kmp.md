---
description: 从暴力字符串匹配出发，理解 KMP 的核心思想、nxt 数组构造、匹配过程、最短循环节与字符串删除应用。
---
...
给定一个字符串 $s$ 和模式串 $p$，每当 $s$ 中出现一个与 $p$ 相同的子串，就将这个子串删除，然后把剩余部分重新拼接起来。重复这个过程，直到字符串中不再出现 $p$，求最后剩下的字符串。

如果每找到一次模式串就直接使用 `erase` 删除，不仅需要重新寻找模式串，删除字符串中间的一段内容也可能搬移大量字符，时间复杂度很高。

考虑从左到右依次加入 $s$ 中的字符，并用一个栈保存当前还没有被删除的字符。每加入一个新字符，只有以这个字符结尾的位置可能新出现模式串，因此可以用 KMP 维护当前匹配长度 $j$。

如果加入字符后有 `j==m`，说明栈顶的 $m$ 个字符正好组成模式串，直接将栈顶回退 $m$ 位即可完成删除。

但是删除以后，当前匹配长度应该恢复成多少呢？如果重新扫描栈里的字符串，复杂度仍然可能退化。所以除了保存字符，还要记录加入每个字符以后对应的 KMP 匹配长度：

```cpp
match[top]=j;
```

删除模式串以后，新的栈顶就是当前字符串的最后一个字符，它保存的 `match[top]` 正好是当前应该恢复的匹配长度。因此只需要写：
...
例如原串为 `ABABABA`，模式串为 `ABA`，扫描过程中栈与匹配长度的变化如下：

```text
读入字符： A    B     A       B    A     B      A
栈中内容： A    AB    空      B    BA    BAB    B
匹配长度： 1    2     0       0    1     2      0
                    删除 ABA                    删除 ABA
```

加入第三个字符时第一次匹配出 `ABA`，删除后栈变空，所以恢复为 `j=0`。读到最后一个字符时又匹配出 `ABA`，删除栈顶三个字符后，栈中还剩下最早保留的 `B`，此时直接恢复它保存的匹配长度 $0$。如果删除让原本分开的两部分重新相邻，也同样可以通过新栈顶保存的状态继续匹配，不需要重新扫描前面的字符。
...
每个字符只会入栈一次、出栈至多一次，KMP 的匹配指针也只进行线性次数的移动。因此时间复杂度为 $\mathcal{O}(|s|+|p|)$，空间复杂度为 $\mathcal{O}(|s|+|p|)$。
