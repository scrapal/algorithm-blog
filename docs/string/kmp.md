...
这道题要不断删掉 $s$ 中出现的模式串 $p$，直到再也找不到 $p$，最后输出剩下的字符串。

最容易想到的做法是每找到一次就用 `erase` 删除。但是删掉中间一段后，后面的字符都要往前移动，还要重新查找下一次出现的位置，肯定会超时。

我们可以从左到右扫描 $s$，用一个栈保存还没有被删掉的字符。每加入一个新字符，只有栈顶这一段可能刚好拼出 $p$，所以可以用 KMP 维护当前匹配到的长度 $j$。

如果加入字符后 `j==m`，说明栈顶的 $m$ 个字符就是 $p$，把栈顶往前退 $m$ 位就删掉了。

不过删完以后，$j$ 应该恢复成多少？重新扫描栈里的字符还是会超时，所以还要顺便保存每个字符入栈后的匹配长度：

```cpp
match[top]=j;
```

删掉模式串后，新的栈顶保存的 `match[top]` 就是现在的匹配长度，直接恢复即可：
...
比如原串是 `AABABAB`，模式串是 `ABA`，可以用下面的动画观察整个扫描过程：

<KmpCensorAnimation />

加入第四个字符时，栈顶第一次拼出 `ABA`。删除后栈里还剩下最开始的 `A`，因此从栈顶保存的状态恢复为 `j=1`。接着读入 `B`、`A`，栈中再次拼出 `ABA`，删除后栈空并恢复为 `j=0`。最后读入的 `B` 无法匹配模式串开头，所以最终只剩下 `B`。

就算一次删除让原本分开的两部分接到了一起，也可以从新栈顶保存的状态继续匹配，不需要再从头扫描。

核心代码如下。这里默认已经按照前文的方法求出了模式串 $p$ 的 `nxt` 数组，并且字符串都使用 `1-based` 下标。`st` 用来保存字符，`match` 用来保存每个栈顶对应的匹配长度。
...
每个字符只会入栈一次，也只会被删除一次，再加上前面求 `nxt` 的过程，时间复杂度为 $\mathcal{O}(|s|+|p|)$，空间复杂度为 $\mathcal{O}(|s|+|p|)$。
